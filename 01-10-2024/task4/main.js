...
function getWeather() {
  const API_KEY = "0b5d228cdf10b68cf867fa02580cf52b";
...
if (weatherLocation) {
  weatherLocation.addEventListener("click", function () {
    //get the current location then send it as paramter here:
    getWeather1(a, b);
  });
} else {
...
function getWeather1(a, b) {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?lat=${a}&lon=${b}&appid=${API_KEY}`
  )
    .then((res) => res.json())
    .then((data) => {
      console.log(data);
